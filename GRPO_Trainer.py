...
class CustomTrainer(Trainer):
    ...
    def __init__(
        ...
        is_kbit_model = getattr(model, "is_loaded_in_8bit", False) or getattr(model, "is_loaded_in_4bit", False)
        if peft_config is not None:
            if not is_peft_available():
                raise ImportError("PEFT is required when `use_peft` is enabled. Install it with `pip install peft`.")
            if is_kbit_model:
        ...
        if args.gradient_checkpointing and not (peft_config is not None and is_kbit_model):
        ...
        if self.use_vllm and not is_vllm_available():
                raise ImportError(
    ...
    def _generate_and_score_completions(
        ...
        if self.use_vllm:
            ...
            self.model.eval()
            with profiling_context(self, "model.generate"), torch.no_grad():
    ...
    def _compute_loss(self, model, inputs):
        ...
        if mode == "train":
            torch.cuda.empty_cache()
            if self.use_vllm and not self.vllm_sleeping:
                self.llm.sleep()
                self.accelerator.wait_for_everyone()
