...
def load_hf_generator(config):
    ...
    is_adapter = os.path.exists(os.path.join(config.model, "adapter_config.json"))
    if is_adapter:
        if AutoPeftModelForCausalLM is None:
            raise ImportError("PEFT is required to evaluate adapter checkpoints. Install `peft`.")
...
def hf_generate(model, tokenizer, prompts, n, temperature, max_tokens, seed, batch_size):
    if seed is not None:
    ...
    for start in tqdm(range(0, len(prompts), batch_size), desc="HF generation"):
        ...
        for i in range(len(batch_prompts)):
            completions = [
                ...
                for text in decoded[i * n:(i + 1) * n]
            ]
            all_outputs.append(SimpleNamespace(outputs=completions))
    return all_outputs
...
def main(global_args,local_configs):
    ...
    for config in local_configs:
        ...
        if config.use_hf_generation:
            llm = load_hf_generator(config)
            outputs = hf_generate(
                llm,
                tokenizer,
                texts,
            ...
            if LLM is None or SamplingParams is None:
                raise ImportError("vLLM is required for this evaluation config. Set `use_hf_generation: true` to use Transformers.")
